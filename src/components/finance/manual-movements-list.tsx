...
import { activeMovementFilters, EMPTY_MOVEMENT_FILTERS, filterManualMovements, manualMovementsCsv, movementExportFilename, movementFilterError, type MovementFilters } from "@/lib/finance-movement-list";
...
export function ManualMovementsList({ records, month, site, loading, error, userId, isAdmin, deletingId, onEdit, onDelete }: {
  records: ManualFinanceRecord[]; month: string; site: string; loading: boolean; error?: string;
  ...
  const categories = useMemo(() => [...new Set([...records.map(record => record.categoria), filters.category].filter(Boolean))].sort((a, b) => a.localeCompare(b, "es")), [records, filters.category]);
  const rows = useMemo(() => filterManualMovements(records, filters, month), [records, filters, month]);
  ...
  function download() {
    ...
    try {
      ...
      link.href = url; link.download = movementExportFilename(site, month, active > 0); link.hidden = true;
      document.body.appendChild(link); link.click();
      setNotice(`Descarga preparada: ${rows.length} movimientos del periodo ${month}.`);
  ...
  return <div className="min-w-0 rounded-3xl border border-white/10 bg-[#15171d] p-4 sm:p-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div className="min-w-0 flex-1 basis-60"><h2 className="flex items-center gap-2 text-xl font-black"><ReceiptText className="h-5 w-5 shrink-0 text-sky-300" />Movimientos manuales</h2><p className="mt-2 text-xs leading-relaxed text-slate-300">Lista del periodo {month} · {site}. Los pagos automáticos de alumnos no están incluidos en esta lista ni en el archivo.</p></div>
    ...
    <div className="mt-5 overflow-x-auto" role="region" aria-label="Lista de movimientos manuales" tabIndex={0}>
      <table className="w-full min-w-[700px] text-left text-sm"><thead className="text-[10px] font-black uppercase tracking-wider text-slate-300"><tr><th scope="col" className="pb-3">Fecha</th><th scope="col" className="pb-3">Tipo</th><th scope="col" className="pb-3">Concepto</th><th scope="col" className="pb-3">Categoría</th><th scope="col" className="pb-3 text-right">Monto</th><th scope="col" className="pb-3 text-right">Acciones</th></tr></thead><tbody className="divide-y divide-white/10">
        {busy ? <tr><td colSpan={6} className="py-10 text-center text-slate-300">{error || <span className="inline-flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />Cargando movimientos…</span>}</td></tr> : rows.map(item => <tr key={item.id}>
