...
type Data={nombre:string;disciplina:string;habilidadesTecnicas:SkillProgress;historialFisico:PhysicalAssessment[];historialHidratacion:HydrationSession[];metasFisicas?:PhysicalGoals;bienestar:WellnessCheckin[]};
...
export function AthleteProgressPage({mode}:{mode:"skills"|"physical"}){
 ...
 useEffect(()=>{void(async()=>{if(!db||!user)return;try{const profile=await getDoc(doc(db,'usuarios',user.uid)),alumnoId=String(profile.data()?.alumnoId||'');if(!alumnoId)throw new Error('Tu cuenta todavía no está vinculada con un expediente de atleta.');const [athlete,wellness]=await Promise.all([getDoc(doc(db,'Alumnos',alumnoId)),mode==='physical'?getDocs(query(collection(db,'BienestarAtletas'),where('alumnoId','==',alumnoId))):Promise.resolve(null)]);if(!athlete.exists())throw new Error('No se encontró tu expediente.');const value=athlete.data(),bienestar=(wellness?.docs||[]).map(item=>item.data() as WellnessCheckin).filter(item=>/^\d{4}-\d{2}-\d{2}$/.test(item.fecha||'')).sort((a,b)=>b.fecha.localeCompare(a.fecha)).slice(0,45);setData({nombre:String(value.nombre||'Atleta'),disciplina:String(value.habilidadesDisciplina||value.disciplina||''),habilidadesTecnicas:(value.habilidadesTecnicas||{}) as SkillProgress,historialFisico:Array.isArray(value.historialFisico)?value.historialFisico:[],historialHidratacion:Array.isArray(value.historialHidratacion)?value.historialHidratacion:[],metasFisicas:(value.metasFisicas||{}) as PhysicalGoals,bienestar})}catch(e){setError(e instanceof Error?e.message:'No se pudo cargar la información.')}})()},[db,user,mode]);
