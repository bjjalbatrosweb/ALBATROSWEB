...
import { Clock3, CreditCard, Loader2, RefreshCw, Search, ShieldCheck } from 'lucide-react';
...
export default function SolicitudesPagoAdminPage() {
  ...
  const solicitudesVisibles = useMemo(() => {
    ...
    return solicitudes.filter((solicitud) =>
      [solicitud.nombre, solicitud.periodo, solicitud.estado, solicitud.id]
  ...
  return (
    <main className="mx-auto w-full max-w-6xl space-y-7 px-4 py-8 lg:px-8">
      <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-muted-foreground">
            Aquí aparecen las solicitudes generadas desde el módulo público. Generarlas no registra un pago automáticamente.
          </p>
        </div>
        <Button variant="outline" onClick={cargarSolicitudes} disabled={cargando}>
          {cargando ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-2 h-4 w-4" />}
          Actualizar
        </Button>
      </header>
      ...
      <div className="relative">
        ...
        <Input value={busqueda} onChange={(event) => setBusqueda(event.target.value)} className="pl-9" placeholder="Buscar alumno, periodo o estado…" />
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {cargando && solicitudes.length === 0 ? (
            ...
            <p className="rounded-xl border border-dashed p-8 text-center text-muted-foreground">No hay solicitudes que coincidan con la búsqueda.</p>
          ) : (
            solicitudesVisibles.map((solicitud) => (
              <div key={solicitud.id} className="flex flex-col justify-between gap-3 rounded-xl border p-4 sm:flex-row sm:items-center">
                <div>
                  <p className="font-black uppercase">{solicitud.nombre}</p>
                  <p className="mt-1 flex items-center gap-1.5 text-sm text-muted-foreground">
                    <Clock3 className="h-3.5 w-3.5" />
                    {solicitud.periodo} · {solicitud.creadaEn ? new Date(solicitud.creadaEn).toLocaleString('es-MX') : 'Registrando fecha'}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <p className="font-black">{moneda(solicitud.monto)}</p>
                  <Badge variant="outline" className={solicitud.estado === 'pendiente' ? 'border-amber-500/40 text-amber-400' : ''}>
                    {solicitud.estado}
                  </Badge>
                </div>
              </div>
