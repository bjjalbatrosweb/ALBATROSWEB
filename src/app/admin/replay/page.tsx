...
export default function TechnicalReplayPage() {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-5 p-4 text-white md:p-8">
        ...
        <section className="grid gap-5 xl:grid-cols-[1fr_360px]">
          <div className="rounded-[28px] border border-white/10 bg-[#050607] p-3 shadow-2xl md:p-5">
            <div className="relative aspect-video overflow-hidden rounded-[22px] border border-white/10 bg-black">
              ...
              {!cameraOn && <div className="absolute inset-0 grid place-items-center text-center"><div><Camera className="mx-auto mb-3 h-12 w-12 text-white/70" /><p className="font-black uppercase text-white/70">Cámara apagada</p><p className="mt-1 text-xs text-white/70">Actívala cuando el área esté lista.</p></div></div>}
          ...
          <aside className="space-y-4 rounded-[26px] border border-white/10 bg-[#090b0d] p-5">
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-black uppercase">Replay automático</p><p className="text-[10px] text-white/70">Repite el último segmento</p></div><select value={segmentSeconds} disabled={recordingMode === "auto"} onChange={(event) => setSegmentSeconds(Number(event.target.value))} className="replay-input w-24">{[4, 6, 8, 10, 15].map((value) => <option key={value} value={value}>{value} s</option>)}</select></div><button type="button" disabled={recordingMode === "manual"} onClick={recordingMode === "auto" ? stopAutoReplay : startAutoReplay} className={`capture-button w-full ${recordingMode === "auto" ? "border-red-300/35 bg-red-500/15" : "border-cyan-300/25 bg-cyan-500/10"}`}>{recordingMode === "auto" ? <Square /> : <Play />}{recordingMode === "auto" ? "Detener replay" : "Iniciar replay continuo"}</button></div>
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-black"><div className="aspect-video"><video ref={autoVideoRef} src={lastAutoUrl || undefined} muted controls playsInline className="h-full w-full object-contain" />{!lastAutoUrl && <div className="-mt-[56.25%] grid aspect-video place-items-center text-center text-[10px] text-white/70">Aquí aparecerá el último replay</div>}</div><button type="button" disabled={!lastAutoBlob} onClick={() => void saveLastAutoReplay()} className="flex min-h-11 w-full items-center justify-center gap-2 border-t border-white/10 bg-white/[.04] text-xs font-black text-white disabled:opacity-30"><Save />Guardar último replay</button></div>
      ...
      {view === "videoteca" && (
        <section className="rounded-[28px] border border-white/10 bg-[#090b0d] p-4 md:p-6"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-wider text-orange-300">Almacenamiento local</p><h2 className="text-2xl font-black uppercase">Videoteca técnica</h2><p className="text-xs text-white/70">{clips.length} clips · {(storageBytes / 1024 / 1024).toFixed(1)} MB en este navegador</p></div><button type="button" onClick={() => setView("captura")} className="capture-button"><Camera />Nueva captura</button></div>{loading ? <div className="grid min-h-80 place-items-center"><Loader2 className="animate-spin text-orange-300" /></div> : clips.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{clips.map((clip) => <article key={clip.id} className="overflow-hidden rounded-[22px] border border-white/10 bg-black/25"><div className="aspect-video bg-black"><video src={clipUrls[clip.id]} muted playsInline preload="metadata" className="h-full w-full object-contain" /></div><div className="p-4"><p className="truncate font-black">{clip.title}</p><p className="mt-1 text-[10px] text-white/70">{clip.athleteName} · {formatReplayTime(clip.durationSeconds)} · {(clip.size / 1024 / 1024).toFixed(1)} MB</p><div className="mt-3 grid grid-cols-3 gap-2"><button type="button" onClick={() => openAnalysis(clip.id)} className="library-button"><Gauge />Analizar</button><button type="button" onClick={() => downloadClip(clip)} className="library-button"><Download />Descargar</button><button type="button" onClick={() => void removeClip(clip)} className="library-button text-red-300"><Trash2 />Eliminar</button></div></div></article>)}</div> : <EmptyReplay title="Videoteca vacía" detail="Los clips que guardes aparecerán aquí." onGo={() => setView("captura")} />}</section>
      )}
...
function EmptyReplay({ title, detail, onGo }: { title: string; detail: string; onGo: () => void }) {
  return <div className="grid min-h-[440px] place-items-center rounded-[26px] border border-dashed border-white/10 bg-[#090b0d] p-8 text-center"><div><UserRound className="mx-auto mb-4 h-12 w-12 text-white/70" /><h2 className="text-xl font-black uppercase">{title}</h2><p className="mt-2 text-sm text-white/70">{detail}</p><button type="button" onClick={onGo} className="mt-5 rounded-2xl bg-orange-400 px-5 py-3 font-black text-orange-950">Ir a captura</button></div></div>;
}
