...
function WallPresentation({ wall, achievements, site, onClose }: { wall: AchievementWallState; achievements: Achievement[]; site: string; onClose: () => void }) {
  ...
  return <div className={`fixed inset-0 z-[100] overflow-hidden text-white ${theme.backdrop}`}><header className="absolute inset-x-0 top-0 z-20 flex items-center justify-between gap-4 p-5 sm:p-8"><div><p className={`text-xs font-black uppercase tracking-[.3em] ${theme.accent}`}>Sede {site}</p><h1 className="text-2xl font-black text-white sm:text-4xl">{wall.titulo}</h1><p className="mt-1 text-sm text-white/55">{wall.subtitulo}</p></div><div className="flex items-center gap-3"><div className="hidden text-right sm:block"><p className="flex items-center gap-2 text-2xl font-black tabular-nums"><Clock3 className={`h-5 w-5 ${theme.accent}`} /> {clock}</p><p className="text-xs text-white/45">{index + 1} de {achievements.length}</p></div><button onClick={onClose} aria-label="Cerrar pantalla TV" className="grid h-12 w-12 place-items-center rounded-xl border border-white/15 bg-white/[.07]"><Minimize2 className="h-5 w-5" /></button></div></header>{current ? <div className="grid h-full place-items-center px-6 pb-28 pt-32"><div className={`grid w-full max-w-6xl items-center gap-8 rounded-[3rem] border border-white/10 bg-black/35 p-6 backdrop-blur md:grid-cols-[.9fr_1.1fr] md:p-10 ${theme.glow}`}><div className="relative mx-auto aspect-square w-full max-w-[480px] overflow-hidden rounded-[2.5rem] border border-white/15 bg-white/[.06]">{current.fotoUrl ? <Image src={current.fotoUrl} alt={`Foto de ${current.nombre}`} fill sizes="480px" unoptimized priority className="object-cover" /> : <div className="grid h-full place-items-center text-white/25"><UserRound className="h-32 w-32" /></div>}<div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black/80 to-transparent" /></div><div className="text-center md:text-left"><div className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-black uppercase tracking-wider ${theme.badge}`}>{kindIcon(current.kind)} {ACHIEVEMENT_KIND_LABELS[current.kind]}</div><h2 className="mt-6 text-4xl font-black leading-none text-white sm:text-6xl lg:text-7xl">{current.nombre}</h2><p className={`mt-5 text-2xl font-black sm:text-4xl ${theme.accent}`}>{current.resultado}</p><p className="mt-4 text-lg text-white/65 sm:text-2xl">{current.evento || current.disciplina || "Academia Albatros"}</p>{current.fecha && <p className="mt-3 text-sm font-bold uppercase tracking-[.2em] text-white/40">{formatDate(current.fecha)}</p>}</div></div></div> : <div className="grid h-full place-items-center"><p className="text-xl font-bold">No hay logros seleccionados.</p></div>}<footer className="absolute inset-x-0 bottom-0 z-20 flex items-center justify-between gap-4 p-5 sm:p-8"><button onClick={() => move(-1)} disabled={achievements.length < 2} className="grid h-12 w-12 place-items-center rounded-full border border-white/15 bg-white/[.07] disabled:opacity-30"><ChevronLeft className="h-6 w-6" /></button><div className="flex items-center gap-2"><button onClick={() => setPlaying((value) => !value)} className={`inline-flex min-h-12 items-center gap-2 rounded-full px-5 font-black ${theme.badge}`}>{playing ? <Pause className="h-5 w-5" /> : <Play className="h-5 w-5" />}{playing ? "Pausar" : "Continuar"}</button></div><button onClick={() => move(1)} disabled={achievements.length < 2} className="grid h-12 w-12 place-items-center rounded-full border border-white/15 bg-white/[.07] disabled:opacity-30"><ChevronRight className="h-6 w-6" /></button></footer></div>;
}
