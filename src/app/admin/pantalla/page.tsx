...
export default function PantallaTV() {
  ...
  useEffect(() => {
    ...
    const pantallaRef = doc(firestore, 'Pantallas', sede);
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        const data = snapshot.data() as EventoPantalla;
        ...
        let foto = data.fotoUrl || '';

        if (data.alumnoId && !foto) {
          try {
            const alumnoSnapshot = await getDoc(
              doc(firestore, 'Alumnos', data.alumnoId)
            );

            if (alumnoSnapshot.exists()) {
              const alumno = alumnoSnapshot.data() as AlumnoPantalla;
              foto = alumno.fotoUrl || alumno.foto || alumno.imagenUrl || '';
            }
          } catch (error) {
        ...
        setFotoUrl(foto);
        setEvento(data);
        setEstado(data.estadoLed || (data.permitido ? 'verde' : 'rojo'));
      ...
      (error) => {
        console.error('Error escuchando la pantalla:', error);
        setConectado(false);
      }
    );
    ...
    switch (estado) {
      case 'verde':
        return {
          ...
          colorFondo: 'from-emerald-950/80 via-black to-black',
          colorBorde: 'border-emerald-500/40',
          colorGlow: 'shadow-[0_0_100px_rgba(16,185,129,0.32)]',
          colorIcono: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30',
          Icono: CheckCircle2,
        };
      case 'amarillo':
        return {
          ...
          colorFondo: 'from-amber-950/80 via-black to-black',
          colorBorde: 'border-amber-400/40',
          colorGlow: 'shadow-[0_0_100px_rgba(251,191,36,0.30)]',
          colorIcono: 'bg-amber-500/15 text-amber-300 border-amber-400/30',
          Icono: AlertTriangle,
        };
      case 'rojo':
        return {
          ...
          subtitulo: evento?.mensajePago || evento?.mensaje || 'Acceso no autorizado',
          colorTexto: 'text-red-500',
          colorFondo: 'from-red-950/85 via-black to-black',
          colorBorde: 'border-red-500/45',
          colorGlow: 'shadow-[0_0_110px_rgba(239,68,68,0.35)]',
          colorIcono: 'bg-red-500/15 text-red-500 border-red-500/30',
          Icono: ShieldX,
        };
      default:
        return {
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0">
        <div
          className={cn(
            'absolute left-1/2 top-1/2 h-[60vw] w-[60vw] -translate-x-1/2 -translate-y-1/2 rounded-full blur-3xl opacity-20',
      ...
      <header className="relative z-10 flex items-center justify-between border-b border-white/10 bg-black/30 px-8 py-5 backdrop-blur-xl md:px-12">
  ...
  <Badge
    variant="outline"
    className={cn(
      'gap-2 border-white/10 bg-black/30 px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em]',
      conectado
        ? 'text-emerald-400'
        : 'text-red-400'
    )}
  ...
  <button
    ...
    className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold uppercase transition hover:scale-105"
  >
    Verde
  </button>
  ...
  <button
    ...
    className="rounded-lg bg-yellow-500 px-3 py-2 text-xs font-bold uppercase text-black transition hover:scale-105"
  >
    Amarillo
  </button>
  ...
  <button
    ...
    className="rounded-lg bg-red-600 px-3 py-2 text-xs font-bold uppercase transition hover:scale-105"
  >
    Rojo
  </button>
</div>
      ...
      <section className="relative z-10 flex min-h-[calc(100vh-89px)] items-center justify-center p-6 md:p-10">
        <div
          key={`${estado}-${evento?.rfid || 'espera'}`}
          className={cn(
            'relative w-full max-w-6xl rounded-[2.25rem] border bg-black/45 p-7 backdrop-blur-2xl md:p-12',
        ...
        >
          <div
            className={cn(
              'absolute inset-x-0 top-0 h-1.5 rounded-t-[2.25rem]',
              estado === 'verde' && 'bg-emerald-500',
              estado === 'amarillo' && 'bg-amber-400',
              estado === 'rojo' && 'bg-red-500',
          ...
          {estado === 'espera' ? (
            <div className="flex min-h-[560px] flex-col items-center justify-center text-center">
              <div
                className={cn(
                  'mb-9 flex h-36 w-36 items-center justify-center rounded-full border animate-pulse',
            ...
            <div className="grid items-center gap-10 lg:grid-cols-[360px_1fr]">
              <div className="flex justify-center">
                <div
                  className={cn(
                    'relative h-72 w-72 overflow-hidden rounded-[2rem] border-4 bg-zinc-950 md:h-80 md:w-80',
                    configuracion.colorBorde,
                    configuracion.colorGlow
                  )}
                >
                  {fotoUrl && !imagenConError ? (
                    ...
                    <div className="flex h-full w-full flex-col items-center justify-center bg-gradient-to-br from-zinc-900 to-black">
                      ...
                      <span className="mt-5 text-6xl font-black italic text-white/35">
              ...
              <div className="text-center lg:text-left">
                <div className="flex flex-col items-center gap-4 lg:flex-row">
                  ...
                  <div>
                    <p className={cn('text-sm font-black uppercase tracking-[0.4em]', configuracion.colorTexto)}>
                      {configuracion.etiqueta}
                    </p>
                    <h2 className={cn('mt-2 text-4xl font-black uppercase italic tracking-tighter md:text-6xl', configuracion.colorTexto)}>
                      {configuracion.titulo}
                    </h2>
                  </div>
                </div>

                <h3 className="mt-8 text-5xl font-black uppercase italic tracking-tighter text-white md:text-7xl">
                  {nombreAlumno}
                </h3>
                <p className="mt-5 text-2xl font-semibold text-white/65 md:text-3xl">
                  {configuracion.subtitulo}
                </p>

                <div className="mt-8 flex flex-wrap justify-center gap-3 lg:justify-start">
                  <Badge variant="outline" className="border-white/10 bg-white/5 px-5 py-2 text-sm font-black uppercase tracking-widest text-white/70">
                    Sede {nombreSede(evento?.sede || sede)}
                  </Badge>

                  {evento?.rfid && (
                    <Badge variant="outline" className="border-white/10 bg-white/5 px-5 py-2 font-mono text-sm font-black text-white/55">
                      RFID {evento.rfid}
                    </Badge>
                  )}

                  {horaEvento && (
                    <Badge variant="outline" className="border-white/10 bg-white/5 px-5 py-2 text-sm font-black text-white/55">
                ...
                <div className="mt-10 h-2 overflow-hidden rounded-full bg-white/10">
                  <div
                    className={cn(
                      'h-full w-full origin-left animate-[pantallaBarra_7s_linear_forwards]',
                      estado === 'verde' && 'bg-emerald-500',
                      estado === 'amarillo' && 'bg-amber-400',
                      estado === 'rojo' && 'bg-red-500'
                    )}
