...
export default function ComprarPage() {
  ...
  const lines = useMemo(
    () =>
      PRODUCTOS.filter((product) => (cart[product.id] || 0) > 0).map(
        (product) => ({
          ...product,
  ...
  return (
    <main className="relative mx-auto w-full max-w-7xl space-y-7 overflow-hidden px-4 py-8 lg:px-8">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -left-40 top-0 -z-10 h-96 w-96 rounded-full bg-red-600/10 blur-3xl"
      />
      ...
      <header className="relative overflow-hidden rounded-3xl border border-red-500/20 bg-gradient-to-br from-red-950/35 via-card to-card px-6 py-7 shadow-2xl sm:px-8 sm:py-9">
        <div
          aria-hidden="true"
          className="absolute -right-16 -top-24 h-64 w-64 rounded-full border-[42px] border-red-500/5"
        />
        <div className="relative max-w-3xl">
          <Badge
            variant="outline"
            className="mb-4 border-red-500/40 bg-red-500/10 px-3 py-1 text-red-400"
          >
            TIENDA ALBATROS · {sede.replace("_", " ")}
          </Badge>
          <h1 className="text-4xl font-black uppercase italic tracking-[-0.04em] text-white sm:text-5xl">
            Carga. Entrena. Repite.
          </h1>
          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-base">
            Elija sus productos, revise el carrito y confirme la compra
            acercando su tarjeta RFID al teléfono Android.
          </p>
          ...
          <CardContent className="flex flex-col items-center gap-4 p-7 text-center sm:flex-row sm:text-left">
            ...
            <div className="flex-1">
              ...
              <p className="text-muted-foreground">
                {resultado.nombre} · {moneda(resultado.total)} · pendiente de
                cobro.
              </p>
            </div>
            <Button variant="outline" onClick={nuevaCompra}>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        <section className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {PRODUCTOS.map((product) => {
            ...
            return (
              <Card
                key={product.id}
                className={`group overflow-hidden border-white/10 bg-gradient-to-br ${product.color} shadow-xl transition-all duration-300 hover:-translate-y-1 hover:border-white/20 hover:shadow-2xl ${quantity ? "border-red-500/60 ring-1 ring-red-500/25" : ""}`}
              >
                  ...
                  <div className="px-2 pb-2 pt-4">
                    ...
                    <div className="mt-4 flex items-center justify-between rounded-xl border border-white/10 bg-black/40 p-1.5">
                      <Button
                        ...
                        disabled={!quantity || estado === "enviando"}
                      ...
                      <Button
                        ...
                        variant="ghost"
                        onClick={() => changeQuantity(product.id, 1)}
                        disabled={quantity >= 20 || estado === "enviando"}
        ...
        <aside className="xl:sticky xl:top-24 xl:self-start">
          <Card className="overflow-hidden border-red-500/25 bg-card/95 shadow-2xl backdrop-blur">
            <CardHeader className="border-b border-white/5 bg-gradient-to-r from-red-950/30 to-transparent">
              <CardTitle className="flex items-center gap-2">
                <span className="rounded-xl bg-red-500/10 p-2 text-red-500">
                  <ShoppingCart className="h-5 w-5" />
                </span>
                Su compra <Badge variant="secondary">{units}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {lines.length === 0 ? (
                ...
                lines.map((line) => (
                  ...
                  >
                    ...
                    <p className="font-black">{moneda(line.subtotal)}</p>
                  </div>
                ))
              )}
              ...
              {(estado === "iniciando" ||
                ...
                <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-5 text-center">
                  {estado === "enviando" ? (
                    ...
                    <ScanLine className="mx-auto mb-3 h-10 w-10 animate-pulse text-emerald-400" />
                  )}
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    La compra solo se confirma después de identificar al alumno.
                  </p>
                </div>
              )}
              ...
              <p className="text-center text-[11px] text-muted-foreground">
                No registra pagos ni descuenta inventario.
              </p>
            </CardContent>
          </Card>
        </aside>
