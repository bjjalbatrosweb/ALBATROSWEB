  ...
  return (
    <div id="class-live-page" className="pb-8 text-white" style={{ color: '#ffffff' }}>
      <style jsx global>{`
        ...
        #class-live-page [class*="text-white/70"],
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex h-11 items-center rounded-full border border-white/10 bg-white/[0.045] p-1">
            <button
              ...
              className={cn('h-9 rounded-full px-4 text-[9px] font-black uppercase tracking-wider transition', musicSource === 'local' ? 'bg-white text-[#08090d] shadow-lg' : 'text-white/70 hover:text-white')}
            ...
            <button
              ...
              className={cn('h-9 rounded-full px-4 text-[9px] font-black uppercase tracking-wider transition', musicSource === 'youtube' ? 'bg-red-600 text-white shadow-lg' : 'text-white/70 hover:text-white')}
      ...
      <div className="relative text-white">
        ...
        >
          ...
          <section className={cn('relative min-w-0 overflow-hidden rounded-[2rem] border border-white/[0.08] bg-[#08090d] shadow-[0_30px_80px_rgba(0,0,0,0.38)] sm:rounded-[2.25rem]', musicSource !== 'local' && 'hidden')}>
            ...
            <div className="relative p-4 sm:p-6 xl:p-8">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div><p className="text-[9px] font-black uppercase tracking-[0.24em] text-red-400">Reproduciendo ahora</p><p className="mt-1 text-xs text-white/70">Directo desde este dispositivo</p></div>
                <div className="flex flex-wrap items-center gap-1.5">
                  <button onClick={() => void linkMusicFolder()} disabled={importing} className="flex h-10 items-center gap-2 rounded-full bg-white px-4 text-[9px] font-black uppercase text-slate-900 shadow-lg transition hover:scale-[1.02] disabled:opacity-45">{importing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Library className="h-4 w-4" />} Carpeta</button>
                  ...
                  <button onClick={() => fileInputRef.current?.click()} disabled={importing} className="flex h-10 items-center gap-2 rounded-full border border-white/10 bg-white/[0.05] px-3 text-[9px] font-black uppercase text-white/70 transition hover:bg-white/10 hover:text-white disabled:opacity-45" title="Guardar una copia permanente dentro del navegador"><Plus className="h-4 w-4" /> Guardar</button>
              ...
              <div className="mx-auto mt-7 max-w-sm px-3">
                ...
                <div className="mt-6 text-center">
                  ...
                  <p className="mt-1 truncate text-sm font-medium text-white/70">{currentTrack?.artist || 'Selecciona una canción de la biblioteca'}</p>
                  {currentTrack && <p className="mt-1 truncate text-[10px] font-bold uppercase tracking-[0.16em] text-white/70">{currentTrack.album} · {currentTrack.sourceFormat || 'Audio local'}</p>}
                </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="mt-1.5 flex justify-between text-[9px] font-semibold tabular-nums text-white/70"><span>{formatTime(currentTime)}</span><span>-{formatTime(Math.max(0, duration - currentTime))}</span></div>
                </div>

                <div className="mt-3 flex items-center justify-center gap-3 sm:gap-4 xl:gap-6">
                  <button onClick={() => setShuffle((value) => !value)} className={cn('grid h-10 w-10 place-items-center rounded-full transition', shuffle ? 'bg-red-500/15 text-red-400' : 'text-white/70 hover:text-white')} aria-label="Reproducción aleatoria"><Shuffle className="h-4 w-4" /></button>
                  ...
                  <button onClick={() => void togglePlayback()} disabled={!currentTrack} className="grid h-16 w-16 place-items-center rounded-full bg-white text-slate-900 shadow-[0_15px_35px_rgba(255,255,255,0.16)] transition hover:scale-105 disabled:opacity-35">{playing ? <Pause className="h-7 w-7 fill-current" /> : <Play className="ml-1 h-7 w-7 fill-current" />}</button>
                ...
                <div className="mx-auto mt-4 flex max-w-[15rem] items-center gap-3"><Volume2 className="h-3.5 w-3.5 text-white/70" /><input type="range" min={0} max={1} step={0.05} value={volume} onChange={(event) => setVolume(Number(event.target.value))} className="h-1 min-w-0 flex-1 accent-white" aria-label="Volumen" /></div>
              </div>
              ...
              <div className="mt-8 rounded-[1.75rem] border border-white/[0.08] bg-white/[0.045] p-3 backdrop-blur-2xl sm:p-4">
                <div className="grid grid-cols-3 gap-1 rounded-2xl bg-black/25 p-1">
                  ...
                  ].map((item) => <button key={item.id} onClick={() => setMusicView(item.id)} className={cn('flex h-10 items-center justify-center gap-1.5 rounded-xl text-[9px] font-black uppercase transition', musicView === item.id ? 'bg-white text-slate-900 shadow-lg' : 'text-white/70 hover:text-white')}><item.icon className="h-3.5 w-3.5" /><span className="hidden sm:inline">{item.label}</span><span className="opacity-45">{item.count}</span></button>)}
                </div>

                {musicView === 'playlist' && <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]"><div className="flex gap-2"><select value={selectedPlaylistId} onChange={(event) => setSelectedPlaylistId(event.target.value)} className="h-10 min-w-0 flex-1 rounded-xl border border-white/10 bg-[#121319] px-3 text-xs font-bold text-white outline-none">{playlists.map((playlist) => <option key={playlist.id} value={playlist.id}>{playlist.name} · {playlist.trackIds.length}</option>)}</select>{playlists.length > 1 && <button onClick={deleteSelectedPlaylist} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 text-white/70 hover:text-red-400"><Trash2 className="h-4 w-4" /></button>}</div><form onSubmit={createPlaylist} className="flex gap-2"><input value={newPlaylistName} onChange={(event) => setNewPlaylistName(event.target.value)} maxLength={40} className="h-10 min-w-0 flex-1 rounded-xl border border-white/10 bg-black/25 px-3 text-xs text-white outline-none focus:border-red-500/50" placeholder="Nueva playlist" /><button type="submit" disabled={!newPlaylistName.trim()} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-red-600 text-white disabled:opacity-35"><Plus className="h-4 w-4" /></button></form></div>}

                <div className="relative mt-3"><Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-white/70" /><input type="search" value={searchValue} onChange={(event) => setSearchValue(event.target.value)} className="h-11 w-full rounded-2xl border border-white/[0.08] bg-black/20 pl-10 pr-10 text-xs text-white outline-none placeholder:text-white/70 focus:border-red-500/35" placeholder="Buscar en tu música" />{searchValue && <button onClick={() => setSearchValue('')} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/70 hover:text-white"><X className="h-4 w-4" /></button>}</div>

                {libraryLoading ? (
                  <div className="grid min-h-52 place-items-center"><Loader2 className="h-6 w-6 animate-spin text-red-500" /></div>
                ) : libraryError ? (
                  <div className="grid min-h-52 place-items-center text-center"><div><Disc3 className="mx-auto h-9 w-9 text-red-500" /><p className="mt-3 text-sm font-black uppercase">Biblioteca no disponible</p><p className="mt-1 max-w-xs text-xs text-white/70">{libraryError}</p><button onClick={() => void loadLocalCatalog()} className="mt-4 h-9 rounded-full border border-white/10 px-4 text-[10px] font-black uppercase">Reintentar</button></div></div>
                ) : visibleTracks.length === 0 ? (
                  <div className="grid min-h-52 place-items-center text-center"><div><Disc3 className="mx-auto h-10 w-10 text-white/15" /><p className="mt-3 text-sm font-black uppercase">{tracks.length ? 'Sin resultados' : 'Tu biblioteca está vacía'}</p><p className="mt-1 max-w-xs text-xs text-white/70">{tracks.length ? 'Prueba otra búsqueda o playlist.' : 'Vincula tu carpeta para escuchar sin copiar ni subir los archivos.'}</p>{!tracks.length && <button onClick={() => void linkMusicFolder()} className="mt-4 h-9 rounded-full bg-white px-5 text-[10px] font-black uppercase text-slate-900">Elegir carpeta</button>}</div></div>
                ) : (
                  <div className="mt-3 max-h-[19rem] space-y-1 overflow-y-auto pr-1 [scrollbar-width:thin]">
                    {displayedTracks.map((track, index) => {
                      ...
                      return (
                        <div key={track.id} className={cn('group flex items-center gap-2.5 rounded-2xl p-2 transition', isCurrent ? 'bg-white/[0.09]' : 'hover:bg-white/[0.05]')}>
                          <span className="w-5 text-center text-[9px] font-bold tabular-nums text-white/70">{String(index + 1).padStart(2, '0')}</span>
                          <button
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-0.5 truncate text-[10px] text-white/70">{track.artist} · {track.album} · {track.source === 'folder' ? 'Carpeta' : track.source === 'session' ? 'Sesión' : formatBytes(track.size)}</p>
                          </div>
                          <button onClick={() => toggleFavorite(track.id)} className={cn('grid h-8 w-8 shrink-0 place-items-center rounded-full transition', isFavorite ? 'text-red-400' : 'text-white/70 hover:bg-white/5 hover:text-white')}><Heart className={cn('h-3.5 w-3.5', isFavorite && 'fill-current')} /></button>
                          {musicView === 'playlist' ? <button onClick={() => removeFromSelectedPlaylist(track.id)} className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-white/70 hover:bg-white/5 hover:text-red-400"><X className="h-3.5 w-3.5" /></button> : <button onClick={() => addToSelectedPlaylist(track.id)} disabled={isInPlaylist} className={cn('grid h-8 w-8 shrink-0 place-items-center rounded-full', isInPlaylist ? 'text-green-400/70' : 'text-white/70 hover:bg-white/5 hover:text-white')}>{isInPlaylist ? <Check className="h-3.5 w-3.5" /> : <Plus className="h-3.5 w-3.5" />}</button>}
                    ...
                    {displayedTracks.length < visibleTracks.length && (
                      <button
                        ...
                        className="mt-2 h-10 w-full rounded-xl border border-white/[0.08] bg-white/[0.035] text-[9px] font-black uppercase tracking-wider text-white/70 transition hover:border-white/15 hover:text-white"
                      >
          ...
          >
            <div className="overflow-hidden rounded-[1.75rem] border border-red-500/15 bg-gradient-to-br from-red-500/[0.09] via-white/[0.025] to-transparent">
              ...
              >
                <span className="flex min-w-0 items-center gap-3">
                  <span className={cn('grid h-11 w-11 shrink-0 place-items-center rounded-2xl border', timerRunning ? 'border-red-400/30 bg-red-500/15 text-red-400 shadow-[0_0_24px_rgba(239,68,68,0.16)]' : 'border-white/[0.08] bg-white/[0.045] text-white/70')}><Clock3 className={cn('h-5 w-5', timerRunning && 'animate-pulse')} /></span>
                ...
                <span className="flex shrink-0 items-center gap-3">
                  ...
                  <ChevronRight className={cn('h-5 w-5 text-white/70 transition-transform duration-300', timerExpanded && 'rotate-90')} />
                </span>
              </button>
              ...
              {timerExpanded && (
                <div className="border-t border-white/[0.07] p-4 sm:p-5">
                  <div className="mb-5">
                    <div className="flex items-center justify-between gap-3"><p className="text-[9px] font-black uppercase tracking-[0.18em] text-white/70">Presets rápidos</p><button type="button" onClick={saveCurrentTimerPreset} disabled={timerLocked || timerWorkDuration <= 0} className="rounded-full border border-white/10 px-3 py-1.5 text-[8px] font-black uppercase text-white/70 transition hover:bg-white/5 hover:text-white disabled:opacity-30">Guardar actual</button></div>
                    <div className="mt-2 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:thin]">{timerPresets.map((preset) => <div key={preset.id} className="flex shrink-0 overflow-hidden rounded-full border border-white/[0.08] bg-white/[0.035]"><button type="button" onClick={() => applyTimerPreset(preset)} disabled={timerLocked} className="h-9 px-3 text-[8px] font-black uppercase text-white/70 transition hover:bg-white/10 hover:text-white disabled:opacity-35">{preset.name}</button>{preset.custom && <button type="button" onClick={() => deleteTimerPreset(preset)} className="grid h-9 w-8 place-items-center border-l border-white/[0.07] text-white/70 hover:bg-red-500/10 hover:text-red-400" aria-label={`Eliminar ${preset.name}`}><X className="h-3 w-3" /></button>}</div>)}</div>
                  </div>
                  <div className="grid items-center gap-5 sm:grid-cols-[11rem_1fr]">
                    <div className="mx-auto text-center">
                      ...
                      <p className="mt-3 text-[9px] font-bold uppercase tracking-wider text-white/70">Total de la serie · {formatTime(timerTotalDuration)}</p>
                    </div>

                    <div className="min-w-0">
                      <div className="grid grid-cols-3 gap-2">
                        <label className="rounded-2xl border border-white/[0.08] bg-black/20 p-3"><span className="block text-[8px] font-black uppercase tracking-wider text-white/70">Roleos</span><input type="number" inputMode="numeric" min={1} max={99} value={timerRounds} disabled={timerLocked} onChange={(event) => setTimerRounds(clampInteger(event.target.value, 1, 99))} className="mt-1 w-full bg-transparent text-xl font-black tabular-nums text-white outline-none disabled:opacity-45" /></label>
                        <label className="rounded-2xl border border-white/[0.08] bg-black/20 p-3"><span className="block text-[8px] font-black uppercase tracking-wider text-white/70">Minutos</span><input type="number" inputMode="numeric" min={0} max={99} value={workMinutes} disabled={timerLocked} onChange={(event) => setWorkMinutes(clampInteger(event.target.value, 0, 99))} className="mt-1 w-full bg-transparent text-xl font-black tabular-nums text-white outline-none disabled:opacity-45" /></label>
                        <label className="rounded-2xl border border-white/[0.08] bg-black/20 p-3"><span className="block text-[8px] font-black uppercase tracking-wider text-white/70">Segundos</span><input type="number" inputMode="numeric" min={0} max={59} value={workSecondsPart} disabled={timerLocked} onChange={(event) => setWorkSecondsPart(clampInteger(event.target.value, 0, 59))} className="mt-1 w-full bg-transparent text-xl font-black tabular-nums text-white outline-none disabled:opacity-45" /></label>
                      </div>

                      <div className="mt-3 rounded-2xl border border-white/[0.08] bg-black/20 p-3">
                        <div className="flex items-center justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-wider text-white/65">Descanso entre roleos</p><p className="mt-0.5 text-[9px] text-white/70">No se añade después del último.</p></div><button type="button" role="switch" aria-checked={restEnabled} disabled={timerLocked} onClick={() => setRestEnabled((value) => !value)} className={cn('relative h-7 w-12 shrink-0 overflow-hidden rounded-full border transition disabled:opacity-45', restEnabled ? 'border-red-400/30 bg-red-500' : 'border-white/10 bg-white/5')}><span className={cn('pointer-events-none absolute left-1 top-1 h-5 w-5 rounded-full bg-white shadow-md transition-transform duration-200', restEnabled ? 'translate-x-5' : 'translate-x-0')} /></button></div>
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          <label className={cn('rounded-xl border border-white/[0.07] bg-white/[0.035] px-3 py-2', !restEnabled && 'opacity-35')}><span className="block text-[8px] font-black uppercase text-white/70">Minutos</span><input type="number" inputMode="numeric" min={0} max={30} value={restMinutes} disabled={!restEnabled || timerLocked} onChange={(event) => setRestMinutes(clampInteger(event.target.value, 0, 30))} className="mt-0.5 w-full bg-transparent text-base font-black tabular-nums outline-none" /></label>
                          <label className={cn('rounded-xl border border-white/[0.07] bg-white/[0.035] px-3 py-2', !restEnabled && 'opacity-35')}><span className="block text-[8px] font-black uppercase text-white/70">Segundos</span><input type="number" inputMode="numeric" min={0} max={59} value={restSecondsPart} disabled={!restEnabled || timerLocked} onChange={(event) => setRestSecondsPart(clampInteger(event.target.value, 0, 59))} className="mt-0.5 w-full bg-transparent text-base font-black tabular-nums outline-none" /></label>
                        </div>
                      </div>

                      <div className="mt-3 grid grid-cols-2 gap-2">
                        <button type="button" role="switch" aria-checked={duckMusicOnRest} onClick={() => setDuckMusicOnRest((value) => !value)} className={cn('rounded-2xl border p-3 text-left transition', duckMusicOnRest ? 'border-red-500/20 bg-red-500/[0.08]' : 'border-white/[0.07] bg-white/[0.025]')}><span className="block text-[8px] font-black uppercase tracking-wider text-white/70">Música en descanso</span><span className={cn('mt-1 block text-[10px] font-black uppercase', duckMusicOnRest ? 'text-red-400' : 'text-white/70')}>{duckMusicOnRest ? 'Bajar al 25 %' : 'Sin cambios'}</span></button>
                        <button type="button" role="switch" aria-checked={pauseMusicOnFinish} onClick={() => setPauseMusicOnFinish((value) => !value)} className={cn('rounded-2xl border p-3 text-left transition', pauseMusicOnFinish ? 'border-red-500/20 bg-red-500/[0.08]' : 'border-white/[0.07] bg-white/[0.025]')}><span className="block text-[8px] font-black uppercase tracking-wider text-white/70">Al terminar</span><span className={cn('mt-1 block text-[10px] font-black uppercase', pauseMusicOnFinish ? 'text-red-400' : 'text-white/70')}>{pauseMusicOnFinish ? 'Pausar música' : 'Seguir sonando'}</span></button>
                      </div>

                      <div className="mt-4 grid grid-cols-[1fr_auto] gap-2">
                        <button type="button" onClick={timerRunning ? pauseTimer : startOrResumeTimer} className={cn('flex h-12 items-center justify-center gap-2 rounded-2xl text-[10px] font-black uppercase tracking-wider shadow-lg transition hover:scale-[1.01]', timerRunning ? 'bg-amber-400 text-slate-900' : 'bg-red-600 text-white shadow-red-950/30')}>{timerRunning ? <Pause className="h-4 w-4 fill-current" /> : <Play className="h-4 w-4 fill-current" />}{timerRunning ? 'Pausar' : timerPhase === 'idle' || timerPhase === 'finished' ? 'Iniciar serie' : 'Continuar'}</button>
                        <button type="button" onClick={resetTimer} disabled={timerPhase === 'idle' && !timerRunning} className="grid h-12 w-12 place-items-center rounded-2xl border border-white/[0.09] bg-white/[0.04] text-white/70 transition hover:bg-white/10 hover:text-white disabled:opacity-25" aria-label="Reiniciar temporizador"><SkipBack className="h-4 w-4" /></button>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {activity && <div className="mt-6">
              ...
              <div className="mt-5 grid grid-cols-3 overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.025] divide-x divide-white/[0.07]"><div className="p-3.5"><Clock3 className="h-4 w-4 text-red-400" /><p className="mt-2 text-lg font-black">{getActivityDuration(activity)}</p><p className="text-[8px] font-black uppercase tracking-wider text-white/70">Minutos</p></div><div className="p-3.5"><Dumbbell className="h-4 w-4 text-red-400" /><p className="mt-2 truncate text-lg font-black">{activity.focus}</p><p className="text-[8px] font-black uppercase tracking-wider text-white/70">Enfoque</p></div><div className="p-3.5"><Activity className="h-4 w-4 text-red-400" /><p className="mt-2 text-lg font-black">{activity.blocks.length}</p><p className="text-[8px] font-black uppercase tracking-wider text-white/70">Bloques</p></div></div>
                ...
                {activity.blocks.map((block, index) => {
                  ...
                  return (
                    <div key={key} className="group relative grid grid-cols-[4.6rem_1fr] gap-3">
                      <div className="relative z-10 flex items-start justify-between pt-4"><span className={cn('text-[9px] font-black tabular-nums', completed ? 'text-green-400/50' : isNext ? 'text-red-400' : 'text-white/70')}>{block.range}</span><span className={cn('mr-1.5 mt-0.5 h-2.5 w-2.5 rounded-full border-2 border-[#0a0b0f] shadow-[0_0_0_1px_rgba(255,255,255,0.1)] transition', completed ? 'bg-green-400' : isNext ? 'bg-red-500 shadow-[0_0_14px_rgba(239,68,68,0.8)]' : 'bg-white/25 group-hover:bg-red-400')} /></div>
                      <div className={cn('rounded-2xl border p-3.5 transition duration-300', completed ? 'border-green-400/15 bg-green-400/[0.045]' : isNext ? 'border-red-500/25 bg-red-500/[0.08] shadow-[0_14px_35px_rgba(127,29,29,0.12)]' : 'border-white/[0.07] bg-white/[0.03] hover:-translate-y-0.5 hover:border-red-500/20 hover:bg-white/[0.055]')}>
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="flex shrink-0 items-center gap-1.5"><span className="rounded-full bg-white/[0.06] px-2 py-1 text-[8px] font-black text-white/70">{block.minutes} min</span><button type="button" onClick={() => toggleActivityBlock(index)} className={cn('grid h-8 w-8 place-items-center rounded-full border transition', completed ? 'border-green-400/25 bg-green-400/15 text-green-400' : 'border-white/10 bg-white/[0.03] text-white/70 hover:border-green-400/25 hover:text-green-400')} aria-label={completed ? `Marcar ${block.title} como pendiente` : `Marcar ${block.title} como completado`}><Check className="h-4 w-4" /></button></div>
                        </div>
                      </div>
                    </div>
                  );
        ...
        <section className="mt-4 overflow-hidden rounded-[1.75rem] border border-white/[0.08] bg-[#0a0b0f] text-white shadow-[0_20px_55px_rgba(0,0,0,0.25)]">
          ...
          {diagnosticsExpanded && <div className="border-t border-white/[0.07] p-4 sm:p-5">
            ...
            ].map((item) => <div key={item.label} className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-3"><div className="flex items-center gap-2"><span className={cn('h-2 w-2 rounded-full', item.ok ? 'bg-green-400' : 'bg-amber-400')} /><p className="text-[8px] font-black uppercase tracking-wider text-white/70">{item.label}</p></div><p className="mt-2 truncate text-[10px] font-bold text-white/70">{item.detail}</p></div>)}</div>
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="text-[9px] text-white/70">Biblioteca: {tracks.length} canciones · {formatBytes(storageUsage)}{storageQuota > 0 ? ` de ${formatBytes(storageQuota)}` : ''}</p><button type="button" onClick={() => void refreshDiagnostics()} className="h-9 rounded-full border border-white/10 px-4 text-[8px] font-black uppercase text-white/70 transition hover:bg-white/5 hover:text-white">Actualizar diagnóstico</button></div>
          </div>}
        </section>
      </div>
    </div>
