...
import { generateQuickPairs, type QuickPair, type QuickProfile } from "@/lib/quick-pairing";
...
export default function QuickPairingRoulettePage() {
  ...
  function spin() { if (participants.length < 2 || spinning) return; setSpinning(true); setPairs([]); setResting([]); window.setTimeout(() => { const result = generateQuickPairs(participants); setPairs(result.pairs); setResting(result.resting); setSpinning(false); }, 750); }
