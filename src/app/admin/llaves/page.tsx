...
function PodiumPlace({ athlete, secondary, place, label, color, icon, featured = false }: { athlete: BracketAthlete | null; secondary?: BracketAthlete | null; place: string; label: string; color: string; icon: React.ReactNode; featured?: boolean }) {
  return <div className={`rounded-3xl border p-5 text-center ${color} ${featured ? "md:-translate-y-3" : ""}`}><div className="mx-auto flex h-10 items-center justify-center">{icon}</div>{athlete ? <><AthletePhoto athlete={athlete} className="mx-auto mt-3 h-20 w-20 rounded-full" /><p className="mt-3 font-black text-white">{athlete.nombre}</p>{secondary && <p className="mt-1 text-xs font-bold text-white/70">y {secondary.nombre}</p>}</> : <div className="mx-auto mt-3 grid h-20 w-20 place-items-center rounded-full border border-dashed border-white/20 text-white/70"><UserRound className="h-8 w-8" /></div>}<p className="mt-3 text-xs font-black uppercase tracking-[.2em]">{place}º · {label}</p></div>;
}
