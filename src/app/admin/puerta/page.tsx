...
export default function PuertaPage() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      }>(`/api/control-puerta?sede=${encodeURIComponent(sede)}`, {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "No se pudo consultar la puerta.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const cambiarEstado = async () => {
    ...
    if (
      next &&
      !window.confirm(
        "¿Liberar la puerta? El imán quedará desactivado y la puerta permanecerá sin bloqueo hasta que apague este switch.",
      )
    )
      return;
    ...
    try {
      ...
      toast({
        ...
        description: data.puertaLiberada
          ? "El ESP32 mantendrá desactivado el imán."
          : "El ESP32 volverá a activar el imán.",
      });
    } catch (cause) {
      const message =
        cause instanceof Error
          ? cause.message
          : "No se pudo cambiar la puerta.";
      setError(message);
      toast({
        variant: "destructive",
        title: "No se cambió la puerta",
        description: message,
      });
    } finally {
      setWorking(false);
    ...
    <main className="mx-auto w-full max-w-4xl space-y-7 py-2 sm:py-5">
      <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          >
            CONTROL REMOTO · {sede.replace("_", " ")}
          </Badge>
          <h1 className="text-3xl font-black uppercase italic tracking-tight sm:text-4xl">
            Puerta
          </h1>
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Controla el imán de la entrada. El estado permanece guardado por
            sede aunque el ESP32 se reinicie.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <Card
        className={`overflow-hidden border-2 transition-all duration-500 ${liberada ? "border-emerald-500/50 bg-emerald-500/[0.05] shadow-[0_0_45px_rgba(16,185,129,0.12)]" : "border-red-500/40 bg-red-500/[0.04]"}`}
      >
        <CardContent className="p-6 sm:p-9">
          <div className="flex flex-col items-center gap-8 text-center sm:flex-row sm:text-left">
            <div
              className={`grid h-20 w-20 shrink-0 place-items-center rounded-3xl ${liberada ? "bg-emerald-500/15 text-emerald-400" : "bg-red-500/15 text-red-400"}`}
            >
              {loading ? (
            ...
            <div className="flex-1">
              ...
              <h2
                className={`mt-2 text-2xl font-black uppercase sm:text-3xl ${liberada ? "text-emerald-400" : "text-red-400"}`}
              >
                {loading
                  ? "Consultando…"
                  : liberada
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                {liberada
                  ? "El imán debe permanecer desactivado. Cualquier persona puede abrir la puerta."
                  : "El imán debe permanecer activo. El acceso funciona normalmente mediante RFID o sensor de salida."}
              </p>
              {actualizadoEn && (
                <p className="mt-3 text-xs text-muted-foreground">
                  Último cambio:{" "}
                  {new Date(actualizadoEn).toLocaleString("es-MX")}
                </p>
              )}
            </div>
            ...
            >
              ...
              >
                {working ? (
                  ...
                  <DoorClosed className="h-6 w-6 text-red-700" />
                )}
              </span>
            </button>
          </div>
        ...
        {liberada && (
          <div className="flex items-center justify-center gap-2 border-t border-amber-500/25 bg-amber-500/10 px-5 py-4 text-center text-xs font-bold text-amber-300">
            <ShieldAlert className="h-4 w-4 shrink-0" />
            La puerta permanecerá físicamente sin bloqueo hasta apagar el
            switch.
          </div>
        )}
      </Card>

      <div className="flex items-start gap-3 rounded-2xl border border-border/70 bg-card/50 p-4 text-xs leading-relaxed text-muted-foreground">
        <Wifi className="mt-0.5 h-4 w-4 shrink-0" />
        <p>
          El cambio se entrega al ESP32 mediante su heartbeat. Si está
          desconectado, conservará el estado anterior hasta volver a conectarse.
        </p>
      </div>
    </main>
  );
