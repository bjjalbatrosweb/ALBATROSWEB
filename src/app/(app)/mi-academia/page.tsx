...
export default function MiAcademiaPage() {
  ...
  useEffect(() => {
    ...
    setNotificacionesAvisos(
      localStorage.getItem("albatrosNotificacionesAtleta") === "true" &&
  ...
  const activarNotificaciones = async () => {
    if (
      ...
      window.alert("Este navegador no permite notificaciones de la aplicación.");
      return;
    }

    const permiso = await Notification.requestPermission();
    const activadas = permiso === "granted";
    localStorage.setItem(
      "albatrosNotificacionesAtleta",
      String(activadas),
    );
    setNotificacionesAvisos(activadas);
    if (!activadas) {
      window.alert("El navegador no concedió permiso para las notificaciones.");
    }
  };
