...
import { DEFAULT_LOW_STOCK, DEFAULT_STORE_STOCK, STORE_PRODUCTS } from '@/lib/store-products';
...
export async function GET(request: Request) {
  try {
    ...
    if (!sede) {
      return NextResponse.json({ ok: false, mensaje: 'La sede no es válida.' }, { status: 400 });
    }

    const compraId = url.searchParams.get('compra')?.trim();
    if (compraId) {
      if (!/^[A-Za-z0-9_-]{20,160}$/.test(compraId)) {
        return NextResponse.json({ ok: false, mensaje: 'La compra no es válida.' }, { status: 400 });
      }
      const snapshot = await adminDb.collection('SolicitudesCompra').doc(compraId).get();
      const data = snapshot.data();
      if (!snapshot.exists || normalizarSede(data?.sede) !== sede) {
        return NextResponse.json({ ok: false, mensaje: 'La compra no existe.' }, { status: 404 });
      }
    ...
    const inventorySnapshot = await adminDb.collection('InventarioTienda').where('sede', '==', sede).get();
    const inventory = new Map(inventorySnapshot.docs.map((document) => [String(document.data().productoId || ''), document.data()]));
    const catalogo = STORE_PRODUCTS.map((product) => {
      ...
      return {
        ...
        existencias: Number.isFinite(Number(saved?.existencias)) ? Number(saved?.existencias) : DEFAULT_STORE_STOCK,
        minimo: Number.isFinite(Number(saved?.minimo)) ? Number(saved?.minimo) : DEFAULT_LOW_STOCK,
    ...
    return NextResponse.json({ ok: false, mensaje: 'No se pudo consultar la tienda.' }, { status: 500 });
  }
}
