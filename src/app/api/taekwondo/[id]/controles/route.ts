...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({
      ok: true,
      controles: snap.docs.map((document) => {
        const data = document.data();
        return {
          id: document.id,
          nombre: String(data.nombre || "Juez"),
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.accion === "revocar") {
      ...
      await ref.update({ controlesActivos: Math.max(1, valid), votosPendientes: [] });
      return NextResponse.json({ ok: true });
    }
    if (body.accion === "vinculacion_general") {
      const vinculacion = await createGeneralLivePairing({ fightRef: ref, hash: hashToken });
      return NextResponse.json({ ok: true, vinculacion });
    }
    if (body.accion === "recuperar_mesa") {
      const control = await createLiveTableControl({ fightRef: ref, hash: hashToken, name: "Mesa recuperada" });
      return NextResponse.json({ ok: true, control });
    }
    const active = await ref.collection("Controles").where("activo", "==", true).get();
    const count = active.docs.filter((document) => document.data().esMesa !== true).length;
    const nombre = String(body.nombre || `Juez ${count + 1}`).trim().slice(0, 30) || `Juez ${count + 1}`;
    const control = await createIndividualLivePairing({ fightRef: ref, hash: hashToken, name: nombre });
    return NextResponse.json({ ok: true, control });
  } catch (error) {
