...
export async function GET(request: Request) {
  try {
    ...
    if (!snapshot.exists) return NextResponse.json({ ok: true, evento: null });

    const data = snapshot.data() || {};
    const fecha = data.fecha instanceof Timestamp ? data.fecha.toDate().toISOString() : null;
    return NextResponse.json({
      ...
      evento: {
        ...
        estadoLed: ['verde', 'amarillo', 'rojo'].includes(data.estadoLed) ? data.estadoLed : 'rojo',
    ...
    return NextResponse.json({ ok: false, mensaje: 'No se pudo consultar la pantalla.' }, { status: 500 });
  }
}
