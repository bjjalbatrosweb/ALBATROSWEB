...
type EstadoCompra = 'pendiente_cobro' | 'preparando' | 'lista' | 'entregada' | 'cancelada';
...
export default function ComprarPage() {
    ...
    const cargarCatalogo = async () => {
      try {
        const { response, data } = await apiRequest<{
          ok?: boolean;
          catalogo?: Producto[];
        }>(`/api/compras?sede=${encodeURIComponent(sede)}`, { cache: 'no-store' });
        if (!cancelled && response.ok && data.ok && Array.isArray(data.catalogo)) {
  ...
  useEffect(() => {
    ...
    void QRCode.toDataURL(
      JSON.stringify({ folio: initial.folio, sede: initial.sede, total: initial.total }),
    ...
    const consultarEstado = async () => {
      try {
        ...
        if (!cancelled && response.ok && data.ok && data.compra) {
          ...
          if (!['entregada', 'cancelada'].includes(updated.estado)) {
            nextPoll = setTimeout(consultarEstado, 6000);
          }
          return;
        }
      } catch {
      ...
      if (!cancelled) nextPoll = setTimeout(consultarEstado, 10000);
    };
  ...
  const lines = useMemo(
    () => catalogo.filter((product) => (cart[product.id] || 0) > 0).map((product) => ({
      ...product,
  ...
  const productosVisibles = useMemo(
    () => filtro === 'todos' ? catalogo : catalogo.filter((product) => product.grupo === filtro),
    [catalogo, filtro],
  );
  const compraBloqueada = ['iniciando', 'esperando_rfid', 'enviando', 'completado'].includes(estado);
  ...
  const changeQuantity = (id: ProductoId, delta: number) => {
    ...
    setCart((current) => {
      ...
      const max = Math.max(0, Math.min(20, product?.activo === false ? 0 : product?.existencias ?? 20));
  ...
  const enviarCompra = async (uid: string) => {
    ...
    try {
      ...
      if (!response.ok || !data.ok || !data.nombre || !data.compraId || !data.folio) {
      ...
      void QRCode.toDataURL(
        JSON.stringify({ folio: compra.folio, sede: compra.sede, total: compra.total }),
        { width: 320, margin: 1, color: { dark: '#050505', light: '#ffffff' } },
      ).then(setQrCompra).catch(() => setQrCompra(''));
      setEstado('completado');
      setCart({});
    } catch (cause) {
      controllerRef.current?.abort();
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#06070a] pb-28 text-white xl:pb-16">
      ...
      <div className="relative mx-auto w-full max-w-[1480px] px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
          ...
          <div className="relative grid gap-8 px-6 py-8 sm:px-9 sm:py-10 lg:grid-cols-[1fr_410px] lg:items-end lg:px-12 lg:py-12">
            <div>
              ...
              <h1 className="max-w-4xl text-4xl font-black uppercase italic leading-[0.92] tracking-[-0.055em] text-white sm:text-6xl lg:text-7xl">
                Elige. Acerca.
                <span className="mt-1 block text-red-500">Sigue entrenando.</span>
              </h1>
              <p className="mt-5 max-w-2xl text-sm leading-6 text-white/50 sm:text-base">
                Selecciona tus productos y confirma la solicitud acercando tu tarjeta RFID. El cobro se realiza directamente en recepción.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 overflow-hidden rounded-2xl border border-white/[0.08] bg-black/25 backdrop-blur-sm">
              {[
                ...
                <div key={item.paso} className={`p-4 sm:p-5 ${index > 0 ? 'border-l border-white/[0.08]' : ''}`}>
                  <span className="text-[10px] font-black tracking-[0.2em] text-red-500">{item.paso}</span>
                  <p className="mt-2 text-xs font-black uppercase text-white sm:text-sm">{item.titulo}</p>
                  <p className="mt-1 hidden text-[10px] leading-4 text-white/35 sm:block">{item.texto}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {resultado && (
          <section className={`mt-6 overflow-hidden rounded-[2rem] border shadow-[0_20px_70px_rgba(0,0,0,.2)] ${
            ...
            <div className="flex flex-col items-center gap-5 p-6 text-center sm:flex-row sm:p-8 sm:text-left">
              <div className={`grid h-16 w-16 shrink-0 place-items-center rounded-2xl border ${
                resultado.estado === 'cancelada'
                  ? 'border-red-400/25 bg-red-400/10 text-red-400'
                  : 'border-emerald-400/25 bg-emerald-400/10 text-emerald-400'
              }`}>
                {resultado.estado === 'entregada' ? <PackageCheck className="h-8 w-8" /> : resultado.estado === 'cancelada' ? <XCircle className="h-8 w-8" /> : <CheckCircle2 className="h-8 w-8" />}
              </div>
              <div className="flex-1">
                <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${resultado.estado === 'cancelada' ? 'text-red-400' : 'text-emerald-400'}`}>
                  Folio {resultado.folio}
                </p>
                <h2 className="mt-1 text-2xl font-black uppercase italic">{estadoCompraLabel(resultado.estado)}</h2>
                <p className="mt-1 text-sm text-white/50">{resultado.nombre} · {moneda(resultado.total)} · sede {resultado.sede.replace('_', ' ')}.</p>
                <div className="mt-4 flex flex-wrap justify-center gap-1.5 sm:justify-start">
                  {(['pendiente_cobro', 'preparando', 'lista', 'entregada'] as EstadoCompra[]).map((step, index) => {
                    const order = ['pendiente_cobro', 'preparando', 'lista', 'entregada'];
                    const reached = resultado.estado !== 'cancelada' && order.indexOf(resultado.estado) >= index;
                    return <span key={step} className={`h-1.5 w-10 rounded-full ${reached ? 'bg-emerald-400' : 'bg-white/10'}`} />;
                  })}
                </div>
              </div>
              {qrCompra && (
                <div className="rounded-2xl bg-white p-2 shadow-xl" title={`Comprobante ${resultado.folio}`}>
                  <Image src={qrCompra} alt={`QR de la compra ${resultado.folio}`} width={104} height={104} unoptimized className="h-[104px] w-[104px]" />
                </div>
              )}
              <div className="flex flex-col gap-2">
                <Button variant="outline" className="h-12 rounded-xl border-white/10 bg-white/5 px-5 hover:bg-white/10" onClick={nuevaCompra}>
                  <RotateCcw className="mr-2 h-4 w-4" />
                  Nueva compra
                </Button>
                <p className="text-center text-[9px] uppercase tracking-wider text-white/25">Estado actualizado automáticamente</p>
              </div>
            </div>
          </section>
        )}
        ...
        <div className="mt-8 grid gap-7 xl:grid-cols-[minmax(0,1fr)_390px]">
          <section aria-labelledby="catalogo-title" className="min-w-0">
            <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.22em] text-red-500">Catálogo</p>
                <h2 id="catalogo-title" className="mt-1 text-2xl font-black uppercase italic tracking-tight sm:text-3xl">
                  Recarga tu energía
                </h2>
              </div>
              ...
              <div className="flex max-w-full gap-1.5 overflow-x-auto rounded-2xl border border-white/[0.07] bg-white/[0.025] p-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {(['todos', 'agua', 'energetica', 'snack'] as FiltroProducto[]).map((option) => {
                  const label = option === 'todos' ? 'Todos' : nombreGrupo(option);
                  return (
                    <button
                      key={option}
                      type="button"
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {productosVisibles.map((product) => {
                ...
                const stockBajo = !agotado && product.existencias <= product.minimo;
                ...
                return (
                  ...
                  >
                    <div className={`relative m-2.5 h-52 overflow-hidden rounded-[1.2rem] bg-gradient-to-br ${product.color}`}>
                    ...
                    <div className="px-5 pb-5 pt-2">
                      <div className="flex items-end justify-between gap-4">
                        <div className="min-w-0">
                          <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/30">{product.detalle}</p>
                          <h3 className="mt-1 truncate text-xl font-black uppercase tracking-tight text-white">{product.nombre}</h3>
                        </div>
                        <span className="shrink-0 text-2xl font-black tracking-[-0.04em] text-white">{moneda(product.precio)}</span>
                      </div>
                      ...
                      {quantity === 0 ? (
                        ...
                        <div className="mt-5 flex h-11 items-center justify-between rounded-xl border border-red-500/25 bg-red-500/[0.08] p-1">
                          <Button type="button" size="icon" variant="ghost" className="h-9 w-9 rounded-lg hover:bg-white/10" onClick={() => changeQuantity(product.id, -1)} disabled={controlesBloqueados} aria-label={`Quitar ${product.nombre}`}>
                            <Minus className="h-4 w-4" />
                          </Button>
                          <div className="text-center">
                            <span className="text-base font-black">{quantity}</span>
                            <span className="ml-1.5 text-[9px] font-black uppercase tracking-wider text-white/35">unidades</span>
                          </div>
                          <Button type="button" size="icon" variant="ghost" className="h-9 w-9 rounded-lg hover:bg-white/10" onClick={() => changeQuantity(product.id, 1)} disabled={controlesBloqueados || quantity >= Math.min(20, product.existencias)} aria-label={`Agregar ${product.nombre}`}>
          ...
          <aside id="resumen-compra" className="scroll-mt-6 xl:sticky xl:top-24 xl:self-start">
            <Card className="overflow-hidden rounded-[2rem] border-white/[0.09] bg-[#101116]/95 shadow-[0_30px_90px_rgba(0,0,0,.5)] backdrop-blur-xl">
              <CardHeader className="border-b border-white/[0.07] bg-gradient-to-r from-red-950/30 to-transparent px-6 py-5">
                <CardTitle className="flex items-center justify-between">
                  ...
                  <Badge className="border border-white/10 bg-white/5 text-white hover:bg-white/5">{units} {units === 1 ? 'unidad' : 'unidades'}</Badge>
                </CardTitle>
              </CardHeader>
              ...
              <CardContent className="space-y-5 p-5 sm:p-6">
                {lines.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-white/10 bg-black/15 px-6 py-10 text-center">
                    ...
                    <p className="mt-4 text-sm font-black uppercase text-white/70">Tu carrito está vacío</p>
                    <p className="mt-1 text-xs leading-5 text-white/35">Agrega productos del catálogo para comenzar.</p>
                  </div>
                ) : (
                  <div className="max-h-[21rem] space-y-2 overflow-y-auto pr-1 [scrollbar-color:rgba(255,255,255,.12)_transparent]">
                    {lines.map((line) => (
                      <div key={line.id} className="flex items-center gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.025] p-2.5">
                        <div className={`relative h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-gradient-to-br ${line.color}`}>
                          <img src={line.imagen} alt="" className="h-full w-full object-contain p-1.5" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-xs font-black uppercase">{line.nombre} · {line.detalle}</p>
                          <p className="mt-1 text-[10px] text-white/35">{line.cantidad} × {moneda(line.precio)}</p>
                        </div>
                        <p className="shrink-0 text-sm font-black">{moneda(line.subtotal)}</p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="rounded-2xl border border-white/[0.07] bg-black/25 p-4">
                  ...
                  <div className="flex items-end justify-between">
                    ...
                    <span className="text-3xl font-black tracking-[-0.05em] text-emerald-400">{moneda(total)}</span>
                  </div>
                </div>
                ...
                {(estado === 'iniciando' || estado === 'esperando_rfid' || estado === 'enviando') && (
                  <div className="relative overflow-hidden rounded-2xl border border-emerald-400/25 bg-emerald-400/[0.08] p-5 text-center">
                    ...
                    <p className="mt-3 text-sm font-black uppercase tracking-wide">
                      {estado === 'enviando' ? 'Registrando compra' : 'Acerca tu tarjeta RFID'}
                    </p>
                    <p className="mt-1 text-[11px] leading-5 text-white/40">
                      {estado === 'enviando' ? 'Estamos enviando tu solicitud a recepción.' : 'Mantén la tarjeta junto al teléfono hasta confirmar.'}
                    </p>
                    {estado !== 'enviando' && (
                      <button type="button" onClick={cancelarLectura} className="mt-3 text-[10px] font-black uppercase tracking-wider text-white/45 underline-offset-4 hover:text-white hover:underline">
                        Cancelar lectura
                      </button>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 border-t border-white/[0.07] pt-5">
                  {[
                    ...
                    <div key={label} className="text-center">
                      ...
                      <p className="mt-2 text-[8px] font-black uppercase leading-3 tracking-wider text-white/25">{label}</p>
                    </div>
                  ))}
                </div>
              </CardContent>
      ...
      {!resultado && (
        <div className="fixed inset-x-3 bottom-3 z-40 xl:hidden">
          <button
            type="button"
            onClick={() => document.getElementById('resumen-compra')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
          ...
          >
            <span className="relative grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-red-600 text-white">
              <ShoppingCart className="h-5 w-5" />
              {units > 0 && <span className="absolute -right-1.5 -top-1.5 grid h-5 min-w-5 place-items-center rounded-full border-2 border-[#14151b] bg-white px-1 text-[9px] font-black text-black">{units}</span>}
            </span>
            <span className="min-w-0 flex-1">
              <span className="block text-[9px] font-black uppercase tracking-[0.16em] text-white/35">Tu compra</span>
              <span className="block text-sm font-black">{lines.length ? `${units} ${units === 1 ? 'producto' : 'productos'}` : 'Selecciona productos'}</span>
            </span>
            <span className="text-xl font-black text-emerald-400">{moneda(total)}</span>
